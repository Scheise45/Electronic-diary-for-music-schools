...
def admin_schedule():
    logger.info(
        f"Начало обработки /admin_schedule для пользователя {current_user.login}, role_id={current_user.role_id}")
    logger.debug(
        f"Атрибуты current_user: login={current_user.login}, role_id={current_user.role_id}, full_name={current_user.full_name}")
    ...
    try:
        ...
        logger.debug(
            f"Найден учебный год: {current_year.name}, id={current_year.id}")
        ...
        try:
            ...
            logger.error(
                f"Ошибка получения предметов: {str(e)}", exc_info=True)
            subjects = []
            ...
            for teacher in teachers:
                try:
                    user = db_session.query(User).filter_by(
                        id=teacher.user_id).first()
                    user_full_name = user.full_name if user else f'Неизвестно (user_id={teacher.user_id})'
                    logger.debug(
                        f"Учитель: id={teacher.id}, user_id={teacher.user_id}, user_full_name={user_full_name}")
                except Exception as e:
                    logger.error(
                        f"Ошибка загрузки user для teacher id={teacher.id}: {str(e)}", exc_info=True)
                    user_full_name = f'Неизвестно (user_id={teacher.user_id})'
            teachers = [t for t in teachers if db_session.query(
                User).filter_by(id=t.user_id).first()]
            logger.debug(
                f"Отфильтровано учителей с пользователями: {len(teachers)}")
        except Exception as e:
        ...
        try:
            ...
            for group in groups:
                logger.debug(
                    f"Группа: class_group={group.class_group}, class_letter={group.class_letter}")
            if not groups:
            ...
            groups = []
            flash(
                'Не удалось загрузить группы. Проверьте настройки базы данных.', 'danger')
            ...
            for lesson in schedule:
                try:
                    subject = db_session.query(Subject).filter_by(
                        id=lesson.subject_id).first()
                    teacher = db_session.query(Teacher).filter_by(
                        id=lesson.teacher_id).first()
                    user = db_session.query(User).filter_by(
                        id=teacher.user_id).first() if teacher else None
                    ...
                    logger.error(
                        f"Ошибка обработки занятия id={lesson.id}: {str(e)}", exc_info=True)
                    continue
            schedule = enriched_schedule
            logger.debug(f"Обогащённое расписание: {len(schedule)} записей")
        except Exception as e:
            logger.error(
                f"Ошибка получения расписания: {str(e)}", exc_info=True)
            schedule = []
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                logger.debug(
                    f"Группа разобрана: class_group={class_group}, class_letter={class_letter}")
            except ValueError:
            ...
            try:
                ...
                logger.error(
                    f"Ошибка добавления занятия: {str(e)}", exc_info=True)
        ...
        logger.error(
            f"Критическая ошибка в admin_schedule: {str(e)}", exc_info=True)
...
def tr_schedule():
    ...
    try:
        ...
        if not teacher:
            logger.error(
                f"Пользователь {current_user.login} (user_id={current_user.id}) не является учителем")
        ...
        logger.debug(
            f"Найдено {len(schedule_items)} записей в расписании для teacher_id={teacher.id}")
        ...
        for s, sub in schedule_items:
            ...
            try:
                ...
                logger.debug(
                    f"Задание для group={group}, subject_id={s.subject_id}, date={selected_date}: {homework_text}")
            except Exception as e:
                logger.error(
                    f"Ошибка поиска домашнего задания для group={group}, subject_id={s.subject_id}: {str(e)}", exc_info=True)
                homework_text = None
            ...
            logger.debug(
                f"Запись расписания: id={s.id}, group={group}, subject={sub.name}, subject_id={s.subject_id}, homework_text={homework_text}")
        ...
        logger.error(
            f"Ошибка рендеринга teacher_schedule.html: {e}", exc_info=True)
...
def homework():
    logger.info(
        f"Попытка открыть /homework для пользователя {current_user.login}, role_id={current_user.role_id}")
    ...
    try:
        ...
        if selected_group and selected_group not in groups:
            logger.warning(
                f"Группа {selected_group} не найдена в списке групп")
            selected_group = None
        ...
        try:
            ...
            if selected_subject_id and not any(s.id == selected_subject_id for s in subjects):
                logger.warning(
                    f"Предмет ID {selected_subject_id} не найден для учителя")
                selected_subject_id = None
        except Exception as e:
            logger.error(
                f"Ошибка получения предметов: {str(e)}", exc_info=True)
            ...
            if not next_lesson:
                ...
                if next_lesson:
                    lesson_date = current_date + \
                        timedelta(days=min_days_ahead + 7)
                else:
                    return None, None
            else:
        ...
        if selected_group and selected_subject_id:
            try:
                ...
                if next_lesson and lesson_date:
                    logger.debug(
                        f"Следующий урок: schedule_id={next_lesson.id}, date={lesson_date}, subject_id={selected_subject_id}")
                    ...
                    if existing_assignment:
                        ...
                        is_editing = True
                        logger.debug(
                            f"Найдено существующее задание: id={existing_assignment.id}, text={existing_homework}")
                else:
                    logger.warning(
                        f"Будущий урок для группы {selected_group}, предмета {selected_subject_id} не найден")
                ...
                logger.error(
                    f"Ошибка поиска урока или задания: {str(e)}", exc_info=True)
                flash('Не удалось найти следующий урок.', 'warning')

        if request.method == 'POST':
            logger.debug(
                "Обработка POST-запроса для добавления/редактирования домашнего задания")
            group = request.form.get('group')
            ...
            try:
                ...
                if not next_lesson or not lesson_date:
                    logger.error(
                        f"Будущий урок для группы {group}, предмета {subject_id} не найден")
                ...
                logger.debug(
                    f"Следующий урок: schedule_id={next_lesson.id}, date={lesson_date}, day_of_week={next_lesson.day_of_week}, lesson_number={next_lesson.lesson_number}, subject_id={subject_id}")
                ...
                if existing_assignment:
                    ...
                    logger.info(
                        f"Обновлено домашнее задание: id={existing_assignment.id}, group={group}, subject_id={subject_id}, date={lesson_date}")
                    ...
                    logger.info(
                        f"Добавлено новое домашнее задание: group={group}, subject_id={subject_id}, date={lesson_date}")
                ...
                logger.error(
                    f"Ошибка добавления/редактирования домашнего задания: {str(e)}", exc_info=True)
...
def diary():
    ...
    if current_user.role_id not in [3, 4]:
        ...
        return render_template(
            ...
            sunday_schedule=[], current_user=current_user,
            selected_date=datetime.now().date(),
            selected_date_str=datetime.now().date().strftime('%Y-%m-%d'),
    ...
    try:
        selected_date = datetime.strptime(
            selected_date, '%Y-%m-%d').date() if selected_date else datetime.now().date()
    except ValueError:
        logger.warning(f"Неверный формат даты: {selected_date}")
        selected_date = datetime.now().date()
        flash('Неверный формат даты.', 'warning')
    ...
    try:
        # Проверка студента
        student = None
        if current_user.role_id == 4:  # Ученик
            student = db_session.query(Student).filter_by(
                user_id=current_user.id).first()
            if not student:
                error_message = 'Вы не зарегистрированы как студент.'
                logger.error(
                    f"Пользователь {current_user.login} (user_id={current_user.id}) не является студентом")
                flash(error_message, 'danger')
                return render_template(
                    'diary.html',
        ...
        elif current_user.role_id == 3:  # Родитель
            # Найти запись родителя по user_id
            parent = db_session.query(Parent).filter_by(
                user_id=current_user.id).first()
            if not parent:
                error_message = 'Вы не зарегистрированы как родитель.'
                logger.error(
                    f"Пользователь {current_user.login} (user_id={current_user.id}) не найден в таблице parents")
                flash(error_message, 'danger')
                return render_template(
                    'diary.html',
                    monday_schedule=schedules[1], tuesday_schedule=schedules[2], wednesday_schedule=schedules[3],
            ...
            # Найти связанного ученика через student_parents
            student_parent = db_session.query(StudentParent).filter_by(
                parent_id=parent.id).first()
            if not student_parent:
                error_message = 'У вас нет связанных учеников.'
                logger.error(
                    f"Для родителя parent_id={parent.id} (user_id={current_user.id}) не найдено связанных учеников в student_parents")
                flash(error_message, 'danger')
                return render_template(
                    'diary.html',
                    monday_schedule=schedules[1], tuesday_schedule=schedules[2], wednesday_schedule=schedules[3],
            ...
            # Найти данные ученика по student_id
            student = db_session.query(Student).filter_by(
                id=student_parent.student_id).first()
            if not student:
                error_message = 'Связанный ученик не найден в базе.'
                logger.error(
                    f"Ученик student_id={student_parent.student_id} не найден для родителя user_id={current_user.id}")
                flash(error_message, 'danger')
                return render_template(
                    'diary.html',
                    monday_schedule=schedules[1], tuesday_schedule=schedules[2], wednesday_schedule=schedules[3],
        ...
        # Проверка учебного года
        current_year = db_session.query(SchoolYear).order_by(
            SchoolYear.id.desc()).first()
        if not current_year:
        ...
        if not student_year:
            ...
            logger.error(
                f"Для студента {student.id} не найден учебный год school_year_id={current_year.id}")
            flash(error_message, 'danger')
            return render_template(
                'diary.html',
                monday_schedule=schedules[1], tuesday_schedule=schedules[2], wednesday_schedule=schedules[3],
